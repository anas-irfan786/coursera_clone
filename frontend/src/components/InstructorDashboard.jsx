...
const MessagesView = () => {
  ...
  const [userSocket, setUserSocket] = useState(null);  // New: user-specific WebSocket
  ...
  useEffect(() => {
    if (!selectedConversation) {
      // Close any existing WebSocket connection if no conversation is selected
      if (socket) {
        try {
          socket.close();
        } catch (error) {
      ...
      ws.onmessage = (event) => {
        try {
          ...
          if (data.type === 'chat_message') {
            ...
            // Only add message if it belongs to the current selected conversation
            if (messageData.conversation_id === selectedConversation?.id) {
              ...
              console.log('Instructor Dashboard: Message for different conversation, not adding to current view. Expected:', selectedConversation?.id, 'Got:', messageData.conversation_id);
            }
  ...
  const sendMessage = async () => {
    ...
    try {
      ...
      // Don't send via WebSocket - the backend already broadcasts the message
